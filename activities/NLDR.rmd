...
One way to visually explore tree sets is to plot a 2- or 3-dimensional representation of treespace using non-linear dimensionality reduction (NLDR). This approach was first suggested for the visualization of phylogenetic trees by Amenta and Klingner [-@Amenta2002] and Hillis et al. [-@Hillis2005], and recently extended by Wilgenbusch et al. [-@Wilgenbusch2016]. The general idea behind NLDR is to find a lower dimensional representation of the relationships among trees that best preserves the true distances between them. 

TreeScaper implements several different methods for calculating tree-to-tree distances such as:

+ Robinson-Foulds (RF) distances [@Robinson1979; @Robinson1981]

+ Matching distances [@Bogdanowicz2012; @Bogdanowicz2013] 

+ Subtree-prune-regraft (SPR) distances [@Whidden2015]]

It uses several stress functions to assess how the original tree-to-tree distances should be optimally represented in lower dimensional space such as:

+ Normalized stress

+ Kruskal-1 stress [@Kruskal1964]

+ Sammon's nonlinear mapping (NLM) stress [@Sammon1969]

+ Curvilinear Components Analysis (CCA) stress [@Demartines1997]]

and several optimization algorithms for finding the best low-dimensional representation given a chosen stress function such as:
...
These data consist of 1,300 trees sampled from individual Bayesian analyses of 13 mitochondrial protein-coding genes in squamates [@Castoe2009]. Each tree was sampled from the posterior distribution of one gene (ie. one hundred trees were sampled from each of the 13 posterior distributions).
